...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    //Write a derived query to list all tickets by specific email
     List<Ticket> finaAllByUserAccountEmail (String email);//finaAllByUserAccount_Email

    //Write a derived query to count how many tickets are sold for a specific movie
    Integer countAllByMovieCinemaMovieName(String name);

    //Write a derived query to list all tickets between a range of dates
    List<Ticket> findAllByDateTimeBetween(LocalDateTime dateTime1, LocalDateTime dateTime2);//findTicketByDateTimeIsBetween

    // ------------------- JPQL QUERIES ------------------- //

    //Write a JPQL query that returns all tickets are bought from a specific user
    @Query("SELECT t FROM Ticket t WHERE t.userAccount.id=?1")
    List<Ticket> fetchTicketsByUserAccount(@Param("userId") Long userId);
   ...
   Integer countAllTicketsByUserAccount(@Param("userId") Long userId);
    ...
    Integer countTicketsByUserInDateRange( @Param("userId") Long userId ,@Param("dateTime1") LocalDateTime dateTime1,
                                           @Param("dateTime2") LocalDateTime dateTime2);
    ...
    //Write a native query to list all tickets where a specific value should be containable in
    // the username or account name or movie name
    @Query(value = "SELECT * FROM ticket t JOIN user_account ua ON t.user_account_id = ua.id " +
